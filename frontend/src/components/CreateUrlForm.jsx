    ...
    <div className="bg-zinc-950 border border-zinc-800 p-6 relative animate-slide-up">
      ...
      <form onSubmit={handleSubmit} className="flex gap-3">
        <input
          ...
          required
          className="neon-input flex-1 bg-black border border-zinc-800 px-4 py-3 font-mono text-sm text-white placeholder-zinc-700 transition-all"
        />
        <button
          type="submit"
          disabled={loading}
          className="bg-green-400 text-black font-mono font-bold text-sm tracking-widest uppercase px-8 py-3 hover:shadow-neon-green transition-all disabled:opacity-50 whitespace-nowrap"
        >
      ...
      {result && (
        <div className={`mt-4 border border-green-400/30 bg-green-400/5 px-4 py-4 flex items-center justify-between ${copied ? 'copy-flash' : ''}`}>
          <div>
            <div className="font-mono text-xs text-zinc-600 uppercase mb-1">Short URL Generated</div>
            <div className="font-mono text-sm neon-text">{result.shortUrl}</div>
          </div>
          <button
            onClick={handleCopy}
            className="font-mono text-xs border border-green-400/40 text-green-400 hover:bg-green-400 hover:text-black transition-all px-4 py-2 ml-4 whitespace-nowrap"
          >
            {copied ? '✓ COPIED' : 'COPY'}
          </button>
        </div>
